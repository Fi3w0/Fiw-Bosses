    ...
    public void tick() {
        ...
        if (tick <= windupTicks) {
            ...
            for (int arm = 0; arm < 3; arm++) {
                ...
                level.sendParticles(PowerParticleOption.create(ParticleTypes.DRAGON_BREATH, 1.0f), px, py, pz, 1, 0.05, 0.05, 0.05, 0.01);
            }
    ...
    private void performSlam(ServerLevel level, Vec3 bossPos) {
        ...
        for (LivingEntity entity : entities) {
            ...
            if (dist <= radius) {
                ...
                entity.hurtServer(level, boss.damageSources().mobAttack(boss), damage * falloff);
        ...
        level.sendParticles(ColorParticleOption.create(ParticleTypes.FLASH, 0xFFFFFFFF), center.x, center.y + 0.5, center.z, 1, 0, 0, 0, 0);

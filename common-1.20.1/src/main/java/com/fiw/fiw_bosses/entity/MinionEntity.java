...
public class MinionEntity extends BossEntity {
    ...
    protected void writeBossCustomData(CompoundTag tag) {
        ...
        if (minionId != null) tag.putString("MinionId", minionId);
        if (ownerBossUuid != null) tag.putUUID("OwnerBoss", ownerBossUuid);
        tag.putString("MovementMode", movementMode);
    }

    @Override
    protected void readBossCustomData(CompoundTag tag) {
        if (!tag.contains("MinionId")) {
            this.discard();
            return;
        }

        this.minionId = tag.getString("MinionId");
        if (tag.hasUUID("OwnerBoss")) {
            this.ownerBossUuid = tag.getUUID("OwnerBoss");
        }
        this.movementMode = tag.contains("MovementMode") ? tag.getString("MovementMode") : "normal";

...
public class ParticleTornadoGoal extends Goal {
    ...
    public void tick() {
        ...
        for (int d = 0; d < disks; d++) {
            double t = (double) d / (disks - 1);
            double diskY    = origin.y + t * height;
            double diskRadius = maxRadius * t;
            ...
            for (int p = 0; p < points; p++) {
                ...
                if (d % 3 == 0) {
                    level.sendParticles(ParticleTypes.SOUL_FIRE_FLAME, px, diskY, pz, 1, 0, 0, 0, 0);
                } else if (d % 3 == 1) {
                    level.sendParticles(ParticleTypes.FLAME, px, diskY, pz, 1, 0, 0, 0, 0.01);
                } else {
                    level.sendParticles(ParticleTypes.SMOKE, px, diskY, pz, 1, 0, 0, 0, 0);
                }
            }
        }

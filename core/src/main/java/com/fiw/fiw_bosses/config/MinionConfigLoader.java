...
public class MinionConfigLoader {
    ...
    public static int reload() {
        ...
        if (files == null || files.length == 0) {
            FiwBossesCore.LOGGER.info("No minion definition files found in {}", minionsDir.getPath());
            return 0;
        }
        ...
        for (File file : files) {
            try (FileReader reader = new FileReader(file)) {
                ...
                if (def.id == null || def.id.isEmpty()) {
                    FiwBossesCore.LOGGER.error("Minion definition in {} has no 'id' field, skipping", file.getName());
                    failed++;
                    continue;
                }
                definitions.put(def.id, def);
                loaded++;
                FiwBossesCore.LOGGER.info("Loaded minion definition: {}", def.id);
            } catch (Exception e) {
                FiwBossesCore.LOGGER.error("Failed to load minion definition from {}: {}", file.getName(), e.getMessage());
                failed++;
            }
        }

        FiwBossesCore.LOGGER.info("Loaded {} minion definitions ({} failed)", loaded, failed);
        return loaded;
    }

    ...
    public static CompletableFuture<SkinData> fetchPlayerSkin(String playerName) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (uuid == null) {
                    FiwBossesCore.LOGGER.warn("Could not find UUID for player: {}", playerName);
                    return null;
                }

                SkinResult result = getSkinResult(uuid);
                if (result == null) {
                    FiwBossesCore.LOGGER.warn("Could not find skin URL for player: {}", playerName);
                    return null;
                }
                ...
                FiwBossesCore.LOGGER.error("Failed to fetch skin for player {}: {}", playerName, e.getMessage());
                return null;
            }
        }, EXECUTOR);
    }
    ...
    public static CompletableFuture<SkinData> fetchLocalSkin(String filename, boolean slim) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (!skinFile.exists()) {
                    FiwBossesCore.LOGGER.warn("Skin file not found: {}", skinFile.getPath());
                    return null;
                }
                byte[] png = Files.readAllBytes(skinFile.toPath());
                return new SkinData(png, slim);
            } catch (Exception e) {
                FiwBossesCore.LOGGER.error("Failed to read skin file {}: {}", filename, e.getMessage());
                return null;
            }
        }, EXECUTOR);
    }

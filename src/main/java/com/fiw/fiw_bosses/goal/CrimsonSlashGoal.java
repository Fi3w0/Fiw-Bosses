...
public class CrimsonSlashGoal extends Goal {
    ...
    public void tick() {
        ...
        if (state == STATE_WINDUP && currentClaw == 0 && tick < nextClawTick) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                level.sendParticles(
                        new DustParticleOptions(new Vector3f(0.8f, 0.0f, 0.2f), 1.0f),
    ...
    private void spawnClawParticles(ServerLevel level, Vec3 pos, int clawIndex) {
        ...
        for (double w = -halfWidth; w <= halfWidth; w += 0.4) {
            ...
            level.sendParticles(
                    new DustParticleOptions(new Vector3f(0.8f, 0.0f, 0.3f), 1.2f),
        ...
        for (double h = 0.5; h <= fanHeight; h += 0.4) {
            level.sendParticles(
                    new DustParticleOptions(new Vector3f(0.9f, 0.0f, 0.2f), 1.0f),
    ...
    private void triggerExplosion(ServerLevel level, Vec3 pos) {
        ...
        level.sendParticles(
                new DustParticleOptions(new Vector3f(0.7f, 0.0f, 0.2f), 2.0f),
        ...
        level.playSound(null, pos.x, pos.y, pos.z,
                SoundEvents.GENERIC_EXPLODE.value(), SoundSource.HOSTILE, 2.0f, 0.7f);
        level.playSound(null, pos.x, pos.y, pos.z,
